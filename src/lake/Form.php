...
class FormBuilder
{
    ...
    public function slider($name, $min, $max, $step, $value = null, $options = [])
    {
        ...
        $options['class'] = isset($options['class']) 
            ? $options['class'] . (stripos($options['class'], 'form-control') !== false ? '' : ' slider form-control') 
            : 'slider form-control';
        return $this->input('text', $name, $value, $options);
    }
...
EOD;
    ...
    public function checkboxs($name, $list, $checked, $options = [])
    {
        ...
        foreach ($list as $k => $v) {
            $options['id'] = "{$name}-{$k}";
            $html[] = sprintf($this->label("{$name}-{$k}", "%s {$v}"), $this->checkbox($name, $k, in_array($k, $checked), $options));
        }
    ...
    public function radios($name, $list, $checked = null, $options = [])
    {
        ...
        foreach ($list as $k => $v) {
            $options['id'] = "{$name}-{$k}";
            $html[] = sprintf($this->label("{$name}-{$k}", "%s {$v}"), $this->radio($name, $k, in_array($k, $checked), $options));
        }
